...
{ 
    ...
    wxStaticText* itemStaticText12 = new wxStaticText( notebookDetailsPanel, ID_TITLEFONTPICKERSTATIC, _( "Title Font Picker" ), wxDefaultPosition, wxDefaultSize, 0 );
    SecondRowHorizontalSizer->Add( itemStaticText12, 1, wxALIGN_CENTER_VERTICAL | wxALL, 5 );

    wxButton* itemButton13 = new wxButton( notebookDetailsPanel, ID_DEFAULTBUTTON, _( "Default" ), wxDefaultPosition, wxDefaultSize, 0 );
...
void PageDetailsDialog::SetDesignTreeID( wxTreeItemId id )
{ 
    if ( id.IsOk( ) )
    {
        m_designTreeID = id;
        ...
        wxArrayString* errors = page->GetErrorArray( );
        if ( !errors->IsEmpty() )
        {
