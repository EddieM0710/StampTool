...
{ 
    ...
    wxFileName::SplitPath( str, &section, &path, &name, &ext, &hasExt );
    if ( hasExt )
    { 
    ...
    Document* ODTDoc( ){ return GetToolData( )->GetODTDocument( ); };
    void SetODTDocument( Document* doc ){ GetToolData( )->SetODTDocument( doc ); };
